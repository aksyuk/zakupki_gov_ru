...
# убираем коды ограничений
DT.restrictions[, restriction.shortName := NULL]
...
# там есть повторы дат, убираем
DT.protocolCancel <- unique(DT.protocolCancel)
dim(DT.protocolCancel)
...
DT.notificationCancel <- unique(DT.notificationCancel)
...
DT.model <- merge(DT.model, DT.protocolsSinglePart, by = 'purchaseNumber',
                  all.x = T)
...
n.msg <- nrow(DT.model[(last.notice.date - frst.notice.date) / 60 / 60 / 24 > 1, ])
